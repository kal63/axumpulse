...
export default function MedicalApplicationStatusPage() {
  ...
  async function fetchStatus() {
    try {
      ...
      if (error?.response?.status === 404) {
        // No application found — show the no-application UI
        setApplication(null)
        setNoApplication(true)
      } else {
  ...
  // Auto-redirect to appropriate dashboard if approved
  // useEffect(() => {
  //   if (application?.status === 'approved') {
  //     const professional = application.professionalType
  //     const dest = professional === 'trainee' ? '/trainer/dashboard' : professional === 'medical' ? '/medical/dashboard' : '/user/dashboard'
  //     const t = setTimeout(() => {
  //       router.push(dest)
  //     }, 1500)
  ...
  if (noApplication) {
    return <NoApplication router={router} />
  }
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <Button
                  onClick={() => {
                    const professional = application.professionalType
                    if (professional === 'trainee') router.push('/trainer/dashboard')
                    else if (professional === 'medical') router.push('/medical/dashboard')
